...
void removerTimer(timer *t) {
    ...
    timer *aux = head;
    while (aux->next != NULL && aux->next != t) aux = aux->next;
...
void atualizarTimer(timer *t, float dt) {
    if (t == NULL) return;
    if (t->ativo == false || t->finalizado == true) return;
...
float valorTimer(const timer *t) {
    if (t == NULL) return 0;
    if (t->modo == REGRESSIVO) return t->duracao - t->decorrido;
    return t->decorrido;
}
...
char *formatarTimer(const timer *t, char *buffer, int tamanho, bool comMs) {
    if (t == NULL || buffer == NULL) return NULL;
